...
import { StreakSettings } from 'src/app/components/specific/gb-streak-card/gb-streak-card.component';
...
export class StreakPage implements OnInit {
  ...
  async parseData(refresh = false, $event?: CustomEvent): Promise<void> {
    ...
    let key = '-';
    this.route.queryParams.subscribe(q => key = q.key);
    ...
    const challenges = this.state.challenges.active.filter(c => (c.name.includes('viz:general') || c.name.includes('viz:streak'))
      && c.name.includes(key));
